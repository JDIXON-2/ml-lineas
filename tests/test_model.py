...
from lineas.models.model_with_hooks import get_model_with_hooks, is_module_name_in_regex
...
def test_init():
    ...
    mwh = get_model_with_hooks(model, model_task="text_generation")
...
def test_find_module_names_with_shapes():
    device = "cpu"
    model, tokenizer = get_model(
        "sshleifer/tiny-gpt2",
        rand_weights=True,
    ...
    # Test if the method can find modules correctly
    module_names = mwh.find_module_names_with_shapes(
        mwh.get_module(), [".*0.mlp.*"], device=device
    )
    print(mwh.get_module())
    assert len(module_names) == 5
    assert module_names["transformer.h.0.mlp.c_proj"] == torch.Size((1, 3, 2))
    assert module_names["transformer.h.0.mlp.c_fc"] == torch.Size((1, 3, 8))
    assert module_names["transformer.h.0.mlp.act"] == torch.Size((1, 3, 8))
    assert module_names["transformer.h.0.mlp.dropout"] == torch.Size((1, 3, 2))
    assert module_names["transformer.h.0.mlp"] == torch.Size((1, 3, 2))
...
def test_stable_diffusion():
    ...
    mwh = get_model_with_hooks(pipe, model_task="text_to_image_generation")
